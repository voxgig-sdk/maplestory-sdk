...
func TestWzn4Direct(t *testing.T) {
	t.Run("direct-load-wzn4", func(t *testing.T) {
		setup := wzn4DirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-wzn4", _mode); _shouldSkip {
			if _reason == "" {
		...
		result, err := client.Direct(map[string]any{
			"path":   "api/wz/img/{region}/{version}/{path}",
			"method": "GET",
			"params": params,
			"query":  query,
		})
...
type wzn4DirectSetupResult struct {
...
func wzn4DirectSetup(mockres any) *wzn4DirectSetupResult {
	loadEnvLocal()
	...
	if live {
		...
		return &wzn4DirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &wzn4DirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
