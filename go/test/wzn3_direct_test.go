...
func TestWzn3Direct(t *testing.T) {
	t.Run("direct-load-wzn3", func(t *testing.T) {
		setup := wzn3DirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-wzn3", _mode); _shouldSkip {
			if _reason == "" {
...
type wzn3DirectSetupResult struct {
...
func wzn3DirectSetup(mockres any) *wzn3DirectSetupResult {
	loadEnvLocal()
	...
	if live {
		...
		return &wzn3DirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &wzn3DirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
