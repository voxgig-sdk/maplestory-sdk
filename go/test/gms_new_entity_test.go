...
func TestGmsNewEntity(t *testing.T) {
	t.Run("instance", func(t *testing.T) {
		testsdk := sdk.TestSDK(nil, nil)
		ent := testsdk.GmsNew(nil)
		if ent == nil {
			t.Fatal("expected non-nil GmsNewEntity")
		}
	})

	t.Run("basic", func(t *testing.T) {
		setup := gms_newBasicSetup(nil)
		...
		for _, _op := range []string{"load"} {
			if _shouldSkip, _reason := isControlSkipped("entityOp", "gms_new." + _op, _mode); _shouldSkip {
				if _reason == "" {
...
func gms_newBasicSetup(extra map[string]any) *entityTestSetup {
	loadEnvLocal()

	_, filename, _, _ := runtime.Caller(0)
	dir := filepath.Dir(filename)

	entityDataFile := filepath.Join(dir, "..", "..", ".sdk", "test", "entity", "gms_new", "GmsNewTestData.json")

	entityDataSource, err := os.ReadFile(entityDataFile)
	if err != nil {
		panic("failed to read gms_new test data: " + err.Error())
	}

	var entityData map[string]any
	if err := json.Unmarshal(entityDataSource, &entityData); err != nil {
		panic("failed to parse gms_new test data: " + err.Error())
	}
	...
	entidEnvRaw := os.Getenv("MAPLESTORY_TEST_GMS_NEW_ENTID")
	...
	idmapResolved := core.ToMapAny(env["MAPLESTORY_TEST_GMS_NEW_ENTID"])
