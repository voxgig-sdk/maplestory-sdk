...
func TestChatEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		chatRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.chat", setup.data)))
		var chatRef01Data map[string]any
		if len(chatRef01DataRaw) > 0 {
			chatRef01Data = core.ToMapAny(chatRef01DataRaw[0][1])
		}
...
func chatBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap := vs.Transform(
		[]any{"chat01", "chat02", "chat03", "api01", "api02", "api03", "region01"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("MAPLESTORY_TEST_CHAT_ENTID")
	...
	idmapResolved := core.ToMapAny(env["MAPLESTORY_TEST_CHAT_ENTID"])
