...
import (
	...
	omni "github.com/voxgig-sdk/maplestory-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.MaplestoryError); is {
				out := map[string]any{
					"name":    "MaplestoryError",
...
func omniCtx(arg any, client *sdk.MaplestorySDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.MaplestorySDK); is {
			client = live
