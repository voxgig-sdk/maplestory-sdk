...
func TestMapDirect(t *testing.T) {
	t.Run("direct-load-map", func(t *testing.T) {
		...
		if setup.live {
			for _, _liveKey := range []string{"frame01", "layer01", "map_id01", "region01", "version01"} {
...
func mapDirectSetup(mockres any) *mapDirectSetupResult {
	...
	env := envOverride(map[string]any{
		"MAPLESTORY_TEST_MAP_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["MAPLESTORY_TEST_MAP_ENTID"]; ok {
