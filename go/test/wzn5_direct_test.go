...
func TestWzn5Direct(t *testing.T) {
	t.Run("direct-load-wzn5", func(t *testing.T) {
		setup := wzn5DirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-wzn5", _mode); _shouldSkip {
			if _reason == "" {
...
type wzn5DirectSetupResult struct {
...
func wzn5DirectSetup(mockres any) *wzn5DirectSetupResult {
	loadEnvLocal()
	...
	if live {
		...
		return &wzn5DirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &wzn5DirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
