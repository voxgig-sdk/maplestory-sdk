...
func TestWzn6Direct(t *testing.T) {
	t.Run("direct-load-wzn6", func(t *testing.T) {
		setup := wzn6DirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-wzn6", _mode); _shouldSkip {
			if _reason == "" {
		...
		result, err := client.Direct(map[string]any{
			"path":   "api/wz/{region}/{version}/{path}",
			"method": "GET",
			"params": params,
			"query":  query,
		})
...
type wzn6DirectSetupResult struct {
...
func wzn6DirectSetup(mockres any) *wzn6DirectSetupResult {
	loadEnvLocal()
	...
	if live {
		...
		return &wzn6DirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &wzn6DirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
