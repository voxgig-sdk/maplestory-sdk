...
func TestPetEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		petRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.pet", setup.data)))
		var petRef01Data map[string]any
		if len(petRef01DataRaw) > 0 {
			petRef01Data = core.ToMapAny(petRef01DataRaw[0][1])
		}
...
func petBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap := vs.Transform(
		[]any{"pet01", "pet02", "pet03", "api01", "api02", "api03", "render01", "render02", "render03", "animated01", "animated02", "animated03", "region01"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("MAPLESTORY_TEST_PET_ENTID")
	...
	idmapResolved := core.ToMapAny(env["MAPLESTORY_TEST_PET_ENTID"])
