...
func TestMusicDirect(t *testing.T) {
	t.Run("direct-load-music", func(t *testing.T) {
		setup := musicDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-music", _mode); _shouldSkip {
			if _reason == "" {
...
func musicDirectSetup(mockres any) *musicDirectSetupResult {
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["MAPLESTORY_TEST_MUSIC_ENTID"]; ok {
