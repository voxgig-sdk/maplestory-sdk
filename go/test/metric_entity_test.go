...
func TestMetricEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		metricRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.metric", setup.data)))
		var metricRef01Data map[string]any
		if len(metricRef01DataRaw) > 0 {
			metricRef01Data = core.ToMapAny(metricRef01DataRaw[0][1])
		}
		...
		if metricRef01DataDt0Loaded == nil {
...
func metricBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap := vs.Transform(
		[]any{"metric01", "metric02", "metric03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("MAPLESTORY_TEST_METRIC_ENTID")
	...
	idmapResolved := core.ToMapAny(env["MAPLESTORY_TEST_METRIC_ENTID"])
