...
[MCP](https://modelcontextprotocol.io) server exposing the Maplestory SDK as
two agent tools — `maplestory_list` and `maplestory_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/maplestory-mcp)
make build

# 2. Provide credentials via the environment
export MAPLESTORY_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user maplestory \
  -- /absolute/path/to/maplestory-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./maplestory-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// maplestory_list: first page of records
{ "entity": "cluster" }
{ "entity": "cluster", "query": { } }

// maplestory_load: one record by id
{ "entity": "android", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/maplestory-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user maplestory \
     -- "$PWD"/dist/*/maplestory-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `maplestory_list` and `maplestory_load` tools now appear
   in new sessions. Ask the agent to *"list cluster using maplestory"*
   and it calls `maplestory_list` with `{"entity":"cluster"}`.
...
```sh
export MAPLESTORY_APIKEY=sk_live_xxx            # API key
export MAPLESTORY_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `maplestory_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `maplestory_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `MAPLESTORY_APIKEY` | API key sent with every request. |
| `MAPLESTORY_BASE` | Optional override of the API base URL. |
...
```sh
./maplestory-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"maplestory_load","arguments":{"entity":"android","query":{"id":1}}}}'
```
