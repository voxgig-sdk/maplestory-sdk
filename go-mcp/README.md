...
MCP server exposing the Maplestory SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `maplestory_list` | `entity`, optional `query` map | First page of records as JSON |
| `maplestory_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `maplestory_list` and
`maplestory_load` tools become available in new sessions.
