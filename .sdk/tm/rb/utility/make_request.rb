...
module MaplestoryUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = MaplestoryResponse.new({})
    ...
    if fetch_err
      ...
      response = MaplestoryResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = MaplestoryResponse.new(fetched)
    else
