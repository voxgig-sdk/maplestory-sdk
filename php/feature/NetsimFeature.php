...
class MaplestoryNetsimFeature extends MaplestoryBaseFeature
{
    ...
    private function _simulate(MaplestoryContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(MaplestoryContext $ctx, array $applied): void
    {
        $client = $this->client;
