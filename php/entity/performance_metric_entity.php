...
// Maplestory SDK PerformanceMetric entity
...
class PerformanceMetricEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new PerformanceMetricEntity($this->_client, $opts);
    }

    /**
     * @param PerformanceMetric|array $args PerformanceMetric data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = MaplestoryHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return PerformanceMetric|array The current PerformanceMetric data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of PerformanceMetric fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = MaplestoryHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of PerformanceMetric fields).
     */
    ...
    /**
     * Load a single PerformanceMetric.
     *
     * @param PerformanceMetricLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed PerformanceMetricLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return PerformanceMetric|array The loaded PerformanceMetric as an assoc-array at the
     *   SDK boundary; throws MaplestoryError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = MaplestoryHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
