...
class Wzn3EntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = MaplestorySDK::test(null, null);
        $ent = $testsdk->Wzn3(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = wzn3_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "wzn3." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        $wzn3_ref01_data = null;
        if (count($wzn3_ref01_data_raw) > 0) {
...
function wzn3_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/wzn3/Wzn3TestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["wzn301", "wzn302", "wzn303", "export01", "export02", "export03", "region01", "version01"] as $k) {
