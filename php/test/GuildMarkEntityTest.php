...
class GuildMarkEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = MaplestorySDK::test(null, null);
        $ent = $testsdk->GuildMark(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = guild_mark_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "guild_mark." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set MAPLESTORY_TEST_GUILD_MARK_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $guild_mark_ref01_data = null;
        if (count($guild_mark_ref01_data_raw) > 0) {
...
function guild_mark_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/guild_mark/GuildMarkTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["guild_mark01", "guild_mark02", "guild_mark03", "api01", "api02", "api03", "background01", "background02", "background03", "mark01", "mark02", "mark03", "region01", "version01"] as $k) {
    ...
    $entid_env_raw = getenv("MAPLESTORY_TEST_GUILD_MARK_ENTID");
