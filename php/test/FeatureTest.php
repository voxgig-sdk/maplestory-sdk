...
class FtCtrl extends MaplestoryControl
{
...
class FtClient
{
    ...
    private MaplestoryUtility $utility;

    public function __construct(MaplestoryUtility $utility, array $options)
    {
    ...
    public function get_utility(): MaplestoryUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (MaplestoryContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public MaplestoryContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (MaplestoryContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = MaplestoryConfig::make_feature($name);
    ...
    private static function buildUrl(MaplestorySpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof MaplestoryError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new MaplestoryResponse($fetched);
        }
    ...
    private function fail(MaplestoryContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (MaplestoryContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
