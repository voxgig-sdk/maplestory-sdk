...
class ItemEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set MAPLESTORY_TEST_ITEM_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function item_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["item01", "item02", "item03", "api01", "api02", "api03", "category01", "category02", "category03", "region01"] as $k) {
    ...
    $entid_env_raw = getenv("MAPLESTORY_TEST_ITEM_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["MAPLESTORY_TEST_ITEM_ENTID"]);
