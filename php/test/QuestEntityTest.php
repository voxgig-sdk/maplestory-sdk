...
class QuestEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set MAPLESTORY_TEST_QUEST_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function quest_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["quest01", "quest02", "quest03", "api01", "api02", "api03", "category01", "category02", "category03", "region01"] as $k) {
    ...
    $entid_env_raw = getenv("MAPLESTORY_TEST_QUEST_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["MAPLESTORY_TEST_QUEST_ENTID"]);
