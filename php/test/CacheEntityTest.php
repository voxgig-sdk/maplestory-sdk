...
class CacheEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        ...
        $ent = $testsdk->Cache(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = cache_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "cache." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set MAPLESTORY_TEST_CACHE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $cache_ref01_data = null;
        if (count($cache_ref01_data_raw) > 0) {
...
function cache_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/cache/CacheTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["cache01", "cache02", "cache03"] as $k) {
    ...
    $entid_env_raw = getenv("MAPLESTORY_TEST_CACHE_ENTID");
