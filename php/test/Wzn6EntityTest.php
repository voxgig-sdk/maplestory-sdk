...
class Wzn6EntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = MaplestorySDK::test(null, null);
        $ent = $testsdk->Wzn6(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = wzn6_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "wzn6." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        $wzn6_ref01_data = null;
        if (count($wzn6_ref01_data_raw) > 0) {
...
function wzn6_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/wzn6/Wzn6TestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["wzn601", "wzn602", "wzn603", "wz01", "wz02", "wz03", "region01", "version01"] as $k) {
