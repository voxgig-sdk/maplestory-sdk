...
class MetricEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set MAPLESTORY_TEST_METRIC_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $metric_ref01_data = null;
        if (count($metric_ref01_data_raw) > 0) {
...
function metric_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["metric01", "metric02", "metric03"] as $k) {
    ...
    $entid_env_raw = getenv("MAPLESTORY_TEST_METRIC_ENTID");
