...
class JobEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set MAPLESTORY_TEST_JOB_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function job_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["job01", "job02", "job03", "api01", "api02", "api03", "skill01", "skill02", "skill03", "skillbook01", "skillbook02", "skillbook03", "region01"] as $k) {
    ...
    $entid_env_raw = getenv("MAPLESTORY_TEST_JOB_ENTID");
