...
class CharacterEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set MAPLESTORY_TEST_CHARACTER_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function character_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["character01", "character02", "character03", "api01", "api02", "api03", "action01", "action02", "action03", "animated01", "animated02", "animated03", "center01", "center02", "center03", "compact01", "compact02", "compact03", "download01", "download02", "download03", "feet_center01", "feet_center02", "feet_center03", "navel_center01", "navel_center02", "navel_center03", "region01"] as $k) {
    ...
    $entid_env_raw = getenv("MAPLESTORY_TEST_CHARACTER_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["MAPLESTORY_TEST_CHARACTER_ENTID"]);
