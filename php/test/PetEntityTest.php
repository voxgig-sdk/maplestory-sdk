...
class PetEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set MAPLESTORY_TEST_PET_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // Bootstrap entity data from existing test data.
        $pet_ref01_data_raw = Vs::items(Helpers::to_map(
            Vs::getpath($setup["data"], "existing.pet")));
        $pet_ref01_data = null;
        if (count($pet_ref01_data_raw) > 0) {
            $pet_ref01_data = Helpers::to_map($pet_ref01_data_raw[0][1]);
        }
...
function pet_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["pet01", "pet02", "pet03", "api01", "api02", "api03", "render01", "render02", "render03", "animated01", "animated02", "animated03", "region01"] as $k) {
    ...
    $entid_env_raw = getenv("MAPLESTORY_TEST_PET_ENTID");
