...
{
    private static function utility(): MaplestoryUtility
    {
        return new MaplestoryUtility();
    }
    ...
    private static function ctx(array $over = []): MaplestoryContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new MaplestoryContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new MaplestoryOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof MaplestoryError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new MaplestoryOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = MaplestoryMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new MaplestoryOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = MaplestoryMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new MaplestoryOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = MaplestoryMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new MaplestorySpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = MaplestoryMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new MaplestoryResponse([]),
            'result' => new MaplestoryResult([]),
        ]);
        [, $err] = MaplestoryMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new MaplestorySpec([]),
            'response' => null,
            'result' => new MaplestoryResult([]),
        ]);
        [, $err] = MaplestoryMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new MaplestorySpec(['step' => 's']),
            'response' => new MaplestoryResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new MaplestoryResult([]),
        ]);
        [, $err] = MaplestoryMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new MaplestorySpec(['step' => 's']),
            'response' => new MaplestoryResponse(self::resp(200, ['v' => 1])),
            'result' => new MaplestoryResult([]),
        ]);
        [, $err] = MaplestoryMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new MaplestorySpec(['step' => 's']),
            'response' => new MaplestoryResponse(self::resp(200, ['v' => 2])),
            'result' => new MaplestoryResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        MaplestoryMakeResponse::call($ctx);
    ...
    {
        $preset = new MaplestoryResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new MaplestorySpec([]),
            'response' => new MaplestoryResponse([]),
            'result' => new MaplestoryResult([]),
        ...
        [$got, $err] = MaplestoryMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new MaplestoryResult([])]);
        [, $err] = MaplestoryMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new MaplestorySpec([]), 'result' => null]);
        [, $err] = MaplestoryMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new MaplestorySpec(['step' => 's']),
            'result' => new MaplestoryResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new MaplestoryOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = MaplestoryMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new MaplestorySpec(['step' => 's']),
            'result' => new MaplestoryResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new MaplestoryOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = MaplestoryMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new MaplestoryResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new MaplestorySpec([]),
            'result' => new MaplestoryResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = MaplestoryMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = MaplestoryMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new MaplestoryError('boom', 'boom');
        $utility->fetcher = function (MaplestoryContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new MaplestorySpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = MaplestoryMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (MaplestoryContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = MaplestoryMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(MaplestoryResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        MaplestoryMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (MaplestoryContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new MaplestorySpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = MaplestoryMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new MaplestoryResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new MaplestorySpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = MaplestoryMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = MaplestoryMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = MaplestoryMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new MaplestoryResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, MaplestoryDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new MaplestoryResult(['ok' => false])]);
        $this->expectException(MaplestoryError::class);
        MaplestoryDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new MaplestoryResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', MaplestoryMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        MaplestoryMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new MaplestoryBaseFeature();
        $b = new MaplestoryBaseFeature();
        MaplestoryFeatureAdd::call($ctx, $a);
        MaplestoryFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): MaplestoryBaseFeature
    {
        $f = new MaplestoryBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        MaplestoryFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): MaplestoryContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new MaplestorySpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = MaplestoryPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        MaplestoryPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new MaplestoryResponse(['headers' => null]),
            'result' => new MaplestoryResult([]),
        ]);
        MaplestoryResultHeaders::call($ctx);
