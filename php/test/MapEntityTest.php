...
class MapEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set MAPLESTORY_TEST_MAP_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function map_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["map01", "map02", "map03", "api01", "api02", "api03", "icon01", "icon02", "icon03", "worldmap01", "worldmap02", "worldmap03", "render01", "render02", "render03", "layer01", "layer02", "layer03", "region01"] as $k) {
    ...
    $entid_env_raw = getenv("MAPLESTORY_TEST_MAP_ENTID");
