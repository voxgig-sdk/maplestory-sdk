// Command Maplestory-cli is a CLI + REPL for the Maplestory SDK,
...
const prompt = "maplestory"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "android avatar cache character chat cluster diff entity1 gms_new guild_mark health item job map metric mob music name npc nxf performance_metric pet quest system tip wzn wzn2 wzn3 wzn4 wzn5 wzn6 z_map"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      MAPLESTORY_APIKEY (api key), MAPLESTORY_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: MAPLESTORY_APIKEY carries the API key and
	// MAPLESTORY_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("MAPLESTORY_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("MAPLESTORY_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewMaplestorySDK(opts)
