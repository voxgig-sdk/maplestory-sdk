...
describe("MetricEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set MAPLESTORY_TEST_METRIC_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local metric_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.metric")))
    local metric_ref01_data = nil
    if #metric_ref01_data_raw > 0 then
      metric_ref01_data = helpers.to_map(metric_ref01_data_raw[1][2])
    end
    ...
    assert.is_nil(err)
    assert.is_not_nil(metric_ref01_data_dt0_loaded)
...
function metric_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("MAPLESTORY_TEST_METRIC_ENTID")
  ...
  local idmap_resolved = helpers.to_map(
    env["MAPLESTORY_TEST_METRIC_ENTID"])
