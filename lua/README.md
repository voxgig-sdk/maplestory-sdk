...
The Lua SDK for the Maplestory API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Android(nil):load({ id = "example_id" }, nil)
...
local result, err = client:Maplestory(nil):load(
  { id = "test01" }, nil
)
