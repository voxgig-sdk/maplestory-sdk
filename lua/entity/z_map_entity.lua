...
local ZMapEntity = {}
ZMapEntity.__index = ZMapEntity


function ZMapEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ZMapEntity)
  self._name = "z_map"
  self._client = client
...
function ZMapEntity:get_name()
  return self._name
end


function ZMapEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ZMapEntity.new(self._client, opts)
end
...
function ZMapEntity:mark_deleted()
  self._deleted = true
end


function ZMapEntity:deleted()
  return true == self._deleted
end


function ZMapEntity:data_set(args)
  if args ~= nil then
...
function ZMapEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ZMapEntity:match_set(args)
  if args ~= nil then
...
function ZMapEntity:stream(action, args, callopts)
...
function ZMapEntity:_run_op(ctx, post_done)
  local utility = self._utility
