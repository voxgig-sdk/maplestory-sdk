...
local TipEntity = {}
TipEntity.__index = TipEntity


function TipEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TipEntity)
  self._name = "tip"
  self._client = client
...
function TipEntity:get_name()
  return self._name
end


function TipEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TipEntity.new(self._client, opts)
end
...
function TipEntity:mark_deleted()
  self._deleted = true
end


function TipEntity:deleted()
  return true == self._deleted
end


function TipEntity:data_set(args)
  if args ~= nil then
...
function TipEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TipEntity:match_set(args)
  if args ~= nil then
...
function TipEntity:stream(action, args, callopts)
...
function TipEntity:_run_op(ctx, post_done)
  local utility = self._utility
