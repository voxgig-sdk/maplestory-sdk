...
local CacheEntity = {}
CacheEntity.__index = CacheEntity


function CacheEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CacheEntity)
  self._name = "cache"
  self._client = client
...
function CacheEntity:get_name()
  return self._name
end


function CacheEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CacheEntity.new(self._client, opts)
end
...
function CacheEntity:mark_deleted()
  self._deleted = true
end


function CacheEntity:deleted()
  return true == self._deleted
end


function CacheEntity:data_set(args)
  if args ~= nil then
...
function CacheEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CacheEntity:match_set(args)
  if args ~= nil then
...
function CacheEntity:match_get()
...
function CacheEntity:stream(action, args, callopts)
...
---@param reqmatch CacheLoadMatch
---@param ctrl? table
---@return Cache
---@return string? err
function CacheEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function CacheEntity:_run_op(ctx, post_done)
  local utility = self._utility
