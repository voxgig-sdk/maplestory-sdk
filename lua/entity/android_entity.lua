...
local AndroidEntity = {}
AndroidEntity.__index = AndroidEntity


function AndroidEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AndroidEntity)
  self._name = "android"
  self._client = client
...
function AndroidEntity:get_name()
  return self._name
end


function AndroidEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AndroidEntity.new(self._client, opts)
end
...
function AndroidEntity:mark_deleted()
  self._deleted = true
end


function AndroidEntity:deleted()
  return true == self._deleted
end


function AndroidEntity:data_set(args)
  if args ~= nil then
...
function AndroidEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AndroidEntity:match_set(args)
  if args ~= nil then
...
function AndroidEntity:match_get()
...
function AndroidEntity:stream(action, args, callopts)
...
function AndroidEntity:_run_op(ctx, post_done)
  local utility = self._utility
