...
function MobEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, MobEntity)
  self._name = "mob"
  self._client = client
...
function MobEntity:get_name()
  return self._name
end


function MobEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return MobEntity.new(self._client, opts)
end
...
function MobEntity:mark_deleted()
  self._deleted = true
end


function MobEntity:deleted()
  return true == self._deleted
end


function MobEntity:data_set(args)
  if args ~= nil then
...
function MobEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function MobEntity:match_set(args)
  if args ~= nil then
...
function MobEntity:stream(action, args, callopts)
...
function MobEntity:_run_op(ctx, post_done)
  local utility = self._utility
