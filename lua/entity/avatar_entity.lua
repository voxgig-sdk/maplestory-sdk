...
local AvatarEntity = {}
AvatarEntity.__index = AvatarEntity


function AvatarEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AvatarEntity)
  self._name = "avatar"
  self._client = client
...
function AvatarEntity:get_name()
  return self._name
end


function AvatarEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AvatarEntity.new(self._client, opts)
end
...
function AvatarEntity:mark_deleted()
  self._deleted = true
end


function AvatarEntity:deleted()
  return true == self._deleted
end


function AvatarEntity:data_set(args)
  if args ~= nil then
...
function AvatarEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AvatarEntity:match_set(args)
  if args ~= nil then
...
function AvatarEntity:match_get()
...
function AvatarEntity:stream(action, args, callopts)
...
---@param reqmatch AvatarLoadMatch
---@param ctrl? table
---@return Avatar
---@return string? err
function AvatarEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function AvatarEntity:_run_op(ctx, post_done)
  local utility = self._utility
