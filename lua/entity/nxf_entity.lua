...
local NxfEntity = {}
NxfEntity.__index = NxfEntity


function NxfEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, NxfEntity)
  self._name = "nxf"
  self._client = client
...
function NxfEntity:get_name()
  return self._name
end


function NxfEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return NxfEntity.new(self._client, opts)
end
...
function NxfEntity:mark_deleted()
  self._deleted = true
end


function NxfEntity:deleted()
  return true == self._deleted
end


function NxfEntity:data_set(args)
  if args ~= nil then
...
function NxfEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function NxfEntity:match_set(args)
  if args ~= nil then
...
function NxfEntity:stream(action, args, callopts)
...
function NxfEntity:_run_op(ctx, post_done)
  local utility = self._utility
