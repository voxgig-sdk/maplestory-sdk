...
local GmsNewEntity = {}
GmsNewEntity.__index = GmsNewEntity


function GmsNewEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GmsNewEntity)
  self._name = "gms_new"
  self._client = client
...
function GmsNewEntity:get_name()
  return self._name
end


function GmsNewEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GmsNewEntity.new(self._client, opts)
end
...
function GmsNewEntity:mark_deleted()
  self._deleted = true
end


function GmsNewEntity:deleted()
  return true == self._deleted
end


function GmsNewEntity:data_set(args)
  if args ~= nil then
...
function GmsNewEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GmsNewEntity:match_set(args)
  if args ~= nil then
...
function GmsNewEntity:match_get()
...
function GmsNewEntity:stream(action, args, callopts)
...
---@param reqmatch GmsNewLoadMatch
---@param ctrl? table
---@return GmsNew
---@return string? err
function GmsNewEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function GmsNewEntity:_run_op(ctx, post_done)
  local utility = self._utility
