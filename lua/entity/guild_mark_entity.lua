...
local GuildMarkEntity = {}
GuildMarkEntity.__index = GuildMarkEntity


function GuildMarkEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GuildMarkEntity)
  self._name = "guild_mark"
  self._client = client
...
function GuildMarkEntity:get_name()
  return self._name
end


function GuildMarkEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GuildMarkEntity.new(self._client, opts)
end
...
function GuildMarkEntity:mark_deleted()
  self._deleted = true
end


function GuildMarkEntity:deleted()
  return true == self._deleted
end


function GuildMarkEntity:data_set(args)
  if args ~= nil then
...
function GuildMarkEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GuildMarkEntity:match_set(args)
  if args ~= nil then
...
function GuildMarkEntity:match_get()
...
function GuildMarkEntity:stream(action, args, callopts)
...
---@param reqmatch GuildMarkLoadMatch
---@param ctrl? table
---@return GuildMark
---@return string? err
function GuildMarkEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function GuildMarkEntity:_run_op(ctx, post_done)
  local utility = self._utility
