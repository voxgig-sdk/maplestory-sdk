...
local DiffEntity = {}
DiffEntity.__index = DiffEntity


function DiffEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DiffEntity)
  self._name = "diff"
  self._client = client
...
function DiffEntity:get_name()
  return self._name
end


function DiffEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DiffEntity.new(self._client, opts)
end
...
function DiffEntity:mark_deleted()
  self._deleted = true
end


function DiffEntity:deleted()
  return true == self._deleted
end


function DiffEntity:data_set(args)
  if args ~= nil then
...
function DiffEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DiffEntity:match_set(args)
  if args ~= nil then
...
function DiffEntity:stream(action, args, callopts)
...
---@param reqmatch DiffLoadMatch
---@param ctrl? table
---@return Diff
---@return string? err
function DiffEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function DiffEntity:_run_op(ctx, post_done)
  local utility = self._utility
