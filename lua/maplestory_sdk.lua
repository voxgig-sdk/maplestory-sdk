...
-- by passing it.
function MaplestorySDK:_raw_request(fetchargs)
  local utility = self._utility
...
function MaplestorySDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "MaplestorySDK: graphql: " .. msg
    res.graphql = errors
  end
