...
(0, node_test_1.describe)('HealthEntity', async () => {
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        if (setup.syntheticOnly) {
            t.skip('live entity test uses synthetic IDs from fixture — set MAPLESTORY_TEST_HEALTH_ENTID JSON to run live');
            return;
        }
...
function basicSetup(extra) {
    ...
    const idmapEnvVal = process.env['MAPLESTORY_TEST_HEALTH_ENTID'];
    const idmapOverridden = null != idmapEnvVal && idmapEnvVal.trim().startsWith('{');
    const env = (0, utility_1.envOverride)({
        'MAPLESTORY_TEST_HEALTH_ENTID': idmap,
        'MAPLESTORY_TEST_LIVE': 'FALSE',
        'MAPLESTORY_TEST_EXPLAIN': 'FALSE',
    });
    idmap = env['MAPLESTORY_TEST_HEALTH_ENTID'];
