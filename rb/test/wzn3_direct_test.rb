...
class Wzn3DirectTest < Minitest::Test
  def test_direct_load_wzn3
    setup = wzn3_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-wzn3", setup[:live] ? "live" : "unit")
    if _should_skip
