...
class Wzn6DirectTest < Minitest::Test
  def test_direct_load_wzn6
    setup = wzn6_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-wzn6", setup[:live] ? "live" : "unit")
    if _should_skip
