...
class MusicEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set MAPLESTORY_TEST_MUSIC_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    music_ref01_data_dt0_loaded, err = music_ref01_ent.load(music_ref01_match_dt0, nil)
    assert_nil err
...
def music_basic_setup(extra)
  ...
  idmap = Vs.transform(
    ["music01", "music02", "music03", "api01", "api02", "api03", "region01", "version01"],
    {
      "`$PACK`" => ["", {
        "`$KEY`" => "`$COPY`",
  ...
  entid_env_raw = ENV["MAPLESTORY_TEST_MUSIC_ENTID"]
