...
class ZMapDirectTest < Minitest::Test
  def test_direct_load_z_map
    setup = z_map_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-z_map", setup[:live] ? "live" : "unit")
    if _should_skip
...
def z_map_direct_setup(mockres)
