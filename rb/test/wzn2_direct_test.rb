...
class Wzn2DirectTest < Minitest::Test
  def test_direct_load_wzn2
    setup = wzn2_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-wzn2", setup[:live] ? "live" : "unit")
    if _should_skip
