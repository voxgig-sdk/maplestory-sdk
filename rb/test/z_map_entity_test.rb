...
class ZMapEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = z_map_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "z_map." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set MAPLESTORY_TEST_Z_MAP_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    z_map_ref01_data = nil
    if z_map_ref01_data_raw.length > 0
      z_map_ref01_data = Helpers.to_map(z_map_ref01_data_raw[0][1])
    end

    # LOAD
    z_map_ref01_ent = client.ZMap(nil)
    z_map_ref01_match_dt0 = {}
    z_map_ref01_data_dt0_loaded, err = z_map_ref01_ent.load(z_map_ref01_match_dt0, nil)
    assert_nil err
    assert !z_map_ref01_data_dt0_loaded.nil?

  end
end

def z_map_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "z_map", "ZMapTestData.json")
  ...
  entid_env_raw = ENV["MAPLESTORY_TEST_Z_MAP_ENTID"]
