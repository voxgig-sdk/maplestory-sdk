...
class GuildMarkDirectTest < Minitest::Test
  def test_direct_load_guild_mark
    setup = guild_mark_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-guild_mark", setup[:live] ? "live" : "unit")
    if _should_skip
...
def guild_mark_direct_setup(mockres)
