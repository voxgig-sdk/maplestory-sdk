...
class Wzn5DirectTest < Minitest::Test
  def test_direct_load_wzn5
    setup = wzn5_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-wzn5", setup[:live] ? "live" : "unit")
    if _should_skip
