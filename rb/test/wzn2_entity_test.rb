...
class Wzn2EntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = wzn2_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "wzn2." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    wzn2_ref01_data = nil
    if wzn2_ref01_data_raw.length > 0
...
def wzn2_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "wzn2", "Wzn2TestData.json")
