...
class Wzn4DirectTest < Minitest::Test
  def test_direct_load_wzn4
    setup = wzn4_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-wzn4", setup[:live] ? "live" : "unit")
    if _should_skip
