...
class PerformanceMetricEntity
  ...
  def make
    opts = @_entopts.dup
    PerformanceMetricEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = MaplestoryHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [PerformanceMetric, Hash] the current PerformanceMetric data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = MaplestoryHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of PerformanceMetric fields)
  def match_get
  ...
  #
  # @param reqmatch [PerformanceMetricLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.PerformanceMetric.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [PerformanceMetric, Hash] the loaded PerformanceMetric; raises MaplestoryError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = MaplestoryHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
