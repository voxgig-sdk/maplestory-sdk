...
class CacheEntity
  ...
  # @return [Cache, Hash] the current Cache data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of Cache fields)
  def match_get
  ...
  # Load a single Cache.
  #
  # @param reqmatch [CacheLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Cache.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Cache, Hash] the loaded Cache; raises MaplestoryError on failure
