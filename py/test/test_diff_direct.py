...
class TestDiffDirect:
    ...
    def test_should_direct_load_diff(self):
        setup = _diff_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-diff", "live" if setup["live"] else "unit")
        if _skip:
...
def _diff_direct_setup(mockres):
