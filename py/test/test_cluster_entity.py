...
class TestClusterEntity:
    ...
    def test_should_stream(self):
        ...
        base = MaplestorySDK.test(seed, None)
        seen = list(base.Cluster(None).stream("list", None, None))
        assert len(seen) == 3

        # Inbound: streaming active -> yields each item from the feature.
        from maplestory_sdk.config import shared_config
        cfg = shared_config()
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = MaplestorySDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.Cluster(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set MAPLESTORY_TEST_CLUSTER_ENTID JSON to run live")
        client = setup["client"]
...
def _cluster_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("MAPLESTORY_TEST_CLUSTER_ENTID"))
