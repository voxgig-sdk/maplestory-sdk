    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set MAPLESTORY_TEST_METRIC_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        metric_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.metric")))
        metric_ref01_data = None
        if len(metric_ref01_data_raw) > 0:
            metric_ref01_data = helpers.to_map(metric_ref01_data_raw[0][1])

        # LOAD
        metric_ref01_ent = client.Metric(None)
        metric_ref01_match_dt0 = {}
        metric_ref01_data_dt0_loaded, err = metric_ref01_ent.load(metric_ref01_match_dt0, None)
        assert err is None
        assert metric_ref01_data_dt0_loaded is not None
...
def _metric_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("MAPLESTORY_TEST_METRIC_ENTID"))
