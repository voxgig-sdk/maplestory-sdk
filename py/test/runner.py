...
class MaplestoryTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            MaplestoryTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = MaplestoryTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = MaplestoryTestRunner.getenv("MAPLESTORY_TEST_LIVE")
        override = MaplestoryTestRunner.getenv("MAPLESTORY_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = MaplestoryTestRunner.getenv(key)
        ...
        explain = MaplestoryTestRunner.getenv("MAPLESTORY_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["MAPLESTORY_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if MaplestoryTestRunner._test_control is not None:
            return MaplestoryTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                MaplestoryTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            MaplestoryTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = MaplestoryTestRunner.load_test_control()
...
def load_env_local():
    MaplestoryTestRunner.load_env_local()


def env_override(m):
    return MaplestoryTestRunner.env_override(m)


def entity_data(v):
    return MaplestoryTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return MaplestoryTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return MaplestoryTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return MaplestoryTestRunner.load_test_control()


def live_client_options():
    return MaplestoryTestRunner.live_client_options()


def live_delay_ms():
    return MaplestoryTestRunner.live_delay_ms()
