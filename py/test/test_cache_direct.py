...
class TestCacheDirect:
    ...
    def test_should_direct_load_cache(self):
        setup = _cache_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-cache", "live" if setup["live"] else "unit")
        if _skip:
...
def _cache_direct_setup(mockres):
