...
class TestZMapDirect:

    def test_should_direct_load_z_map(self):
        setup = _z_map_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-z_map", "live" if setup["live"] else "unit")
        if _skip:
...
def _z_map_direct_setup(mockres):
