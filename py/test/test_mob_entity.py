...
class TestMobEntity:
    ...
    def test_should_create_instance(self):
        ...
        ent = testsdk.Mob(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _mob_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "mob." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set MAPLESTORY_TEST_MOB_ENTID JSON to run live")
        client = setup["client"]
        ...
        mob_ref01_data = None
        if len(mob_ref01_data_raw) > 0:
            mob_ref01_data = helpers.to_map(mob_ref01_data_raw[0][1])

        # LOAD
        mob_ref01_ent = client.Mob(None)
        mob_ref01_match_dt0 = {
            "id": mob_ref01_data["id"],
        }
        mob_ref01_data_dt0_loaded = mob_ref01_ent.load(mob_ref01_match_dt0, None)
        mob_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(mob_ref01_data_dt0_loaded))
        assert mob_ref01_data_dt0_load_result is not None
        assert mob_ref01_data_dt0_load_result["id"] == mob_ref01_data["id"]



def _mob_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/mob/MobTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("MAPLESTORY_TEST_MOB_ENTID"))
