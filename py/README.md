# Maplestory Python SDK

The Python SDK for the Maplestory API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Android(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.Maplestory(None).load(
    {"id": "test01"}, None
)
...
```
py/
├── maplestory_sdk.py         -- Main SDK module
